...
Node* mentionBibliographyAuthors(FuncNode* func, Array<String> const& names) {
...
Array<Node*> mentionBibliography(FuncNode* func, ResourceNode* resource) {
	Array<Node*> items;
	...
	if (func->arguments.has("author")) {
		...
		items.append(mentionBibliographyAuthors(func, names));
	}
...
Array<Node*> mention(ResourceNode* resource, FuncNode* func) {
	...
	if (func->symbol == "bibliography") {
		return mentionBibliography(func, resource);
	}
	return items;
}


Node* genBibliographyAuthors(FuncNode* func, Array<String> const& names) {
	ContentNode* text = new ContentNode();
	text->kind = NodeKind_text;
	text->range = func->range;
...
Array<Node*> genBibliography(FuncNode* func) {
	Array<Node*> items;
	if (func->arguments.has("author")) {
		Array<String> names;
		names.append(func->arguments["author"]);
		items.append(genBibliographyAuthors(func, names));
	}
	if (func->arguments.has("authors")) {
		Array<String> names = _splitNames(func->arguments["authors"]);
		items.append(genBibliographyAuthors(func, names));
