...
namespace HtmlGen {
	...
	struct Generator {
		...
		bool _isLinkCollected;

		Linker _linker;
		Option _option;

		Generator(Node* node, Option const& option) {
			_node = node;
			_activeTable = nullptr;
			_option = option;
			_isLinkCollected = false;
		}
			...
			for (int i = 0; i < sections.size(); i++) {
				...
				if (section.subsections.size() != 0) {
					...
					for (int j = 0; j < section.subsections.size(); j++) {
						...
						content.append("<li class=\"subsection\">\n");
						content.append("<a href=\"#");
						content.append(sectionId);
						content.append(_escapeId(" > "));
						content.append(subsectionId);
						content.append("\">");
						content.append(subsection.title);
						if (subsection.subtitle.size() != 0) {
							content.append(": ");
							content.append(subsection.subtitle);
						}
						content.append("</a>\n");
						if (subsection.subsubsections.size() != 0) {
							...
							for (int k = 0; k < subsection.subsubsections.size(); k++) {
								...
								content.append("<li class=\"subsubsection\">\n");
								content.append("<a href=\"#");
								content.append(sectionId);
								content.append(_escapeId(" > "));
								content.append(subsectionId);
								content.append(_escapeId(" > "));
								content.append(subsubsectionId);
								content.append("\">");
								content.append(subsubsection.title);
								if (subsubsection.subtitle.size() != 0) {
									content.append(": ");
									content.append(subsubsection.subtitle);
								}
								content.append("</a>\n");
								content.append("</li>\n");
							}
		...
		String genMetadata() {
			...
			if (_node->kind == NodeKind_document) {
				String content;
				DocumentNode* doc = static_cast<DocumentNode*>(_node);
				for (auto it = doc->metadata.iterate(); it.hasNext(); it.next()) {
					content.append("<meta name=\"");
					content.append(_escape(it.key()));
					content.append("\" value=\"");
					content.append(_escape(it.value()));
					content.append("\">\n");
				}
				return content;
			}
		...
		String genTitle() {
			...
			if (_node->kind == NodeKind_document) {
				DocumentNode* doc = static_cast<DocumentNode*>(_node);
				if (doc->metadata.has("title")) {
					return doc->metadata["title"];
				}
			}
			Array<Section> sections = _linker.makeOutline();
			if (sections.size() != 0) {
				String title = sections[0].title;
				if (sections[0].subtitle.size() != 0) {
					title.append(": ");
					title.append(sections[0].subtitle);
				}
				return title;
			}
			return "";
		...
		void _gen(Node* node) {
			switch (node->kind) {
				case NodeKind_text: {
					...
					_content.append(_escape(n->content));
					break;
				}
				case NodeKind_space: {
					_content.append("\n");
				...
				case NodeKind_endash: {
					_content.append("&ndash;");
					break;
				}
				case NodeKind_emdash: {
					_content.append("&mdash;");
					break;
				}
				case NodeKind_rightquote: {
					_content.append("&rdquo;");
					break;
				}
				case NodeKind_leftquote: {
					_content.append("&ldquo;");
					break;
				}
				case NodeKind_singlerightquote: {
					_content.append("&rsquo;");
					break;
				}
				case NodeKind_singleleftquote: {
					_content.append("&lsquo;");
					break;
				}
				...
				case NodeKind_hashtag: {
					...
					_content.append("<a class=\"hashtag\" href=\"");
					_content.append(_escape(contentNode->content));
					_content.append("\">#");
					_content.append(_escape(contentNode->content));
					_content.append("</a>");
					break;
				...
				case NodeKind_emoji: {
					...
					if (emoji.size() == 0) {
						_content.append(":");
						_content.append(contentNode->content);
						_content.append(":");
						break;
					}
					_content.append(emoji);
					break;
				}
				...
				case NodeKind_subsubsection: {
					...
					switch (hierarchyNode->kind) {
						case NodeKind_section:
							_sectionIndex++;
							_subsectionIndex = -1;
							_content.append("<section class=\"section\">\n");
							break;
						case NodeKind_subsection:
							_subsectionIndex++;
							_content.append("<section class=\"subsection\">\n");
							break;
						case NodeKind_subsubsection:
							_content.append("<section class=\"subsubsection\">\n");
							break;
						default:
							break;
					}
				...
				case NodeKind_title:
					if (_sectionStacks.size() != 0) {
						...
						for (int i = 0; i < _sectionTitleStacks.size(); i++) {
							if (i != 0) {
								_content.append(_escapeId(" > "));
							}
							_content.append(_escapeId(_sectionTitleStacks[i]));
						}
						_content.append("\">");
				...
				case NodeKind_view: {
					...
					if (ext == ".jpg" || ext == ".jpeg" || ext == ".png" || ext == ".gif") {
						...
						_content.append(_escape(res->source));
						_content.append("\">");
						break;
					}
					_genLink(res);
				...
				case NodeKind_mention: {
					...
					_content.append(_escapeId(result->source));
					_content.append("\">");
				...
				case NodeKind_footnote: {
					ResourceNode* res = static_cast<ResourceNode*>(node);
					_content.append("<sup><a class=\"footnote\" href=\"#Footnote-");
					_content.append(longToString(_footnotes.size() + 1));
					_content.append("\">");
					_content.append(longToString(_footnotes.size() + 1));
				...
				case NodeKind_admon: {
					...
					if (admon->type == "NOTE") {
						_content.append("<div class=\"admonition note\">\n");
						_content.append("<div class=\"label\">Note</div>\n");
					} else if (admon->type == "HINT") {
						_content.append("<div class=\"admonition hint\">\n");
						_content.append("<div class=\"label\">Hint</div>\n");
					} else if (admon->type == "IMPORTANT") {
						_content.append("<div class=\"admonition important\">\n");
						_content.append("<div class=\"label\">Important</div>\n");
					} else if (admon->type == "WARNING") {
						_content.append("<div class=\"admonition warning\">\n");
						_content.append("<div class=\"label\">Warning</div>\n");
					} else if (admon->type == "SEE") {
						_content.append("<div class=\"admonition see\">\n");
						_content.append("<div class=\"label\">See</div>\n");
					} else if (admon->type == "TIP") {
						_content.append("<div class=\"admonition tip\">\n");
						_content.append("<div class=\"label\">Tip</div>\n");
					} else {
						_content.append("<div class=\"admonition note\">\n");
						_content.append("<div class=\"label\">Note</div>\n");
					}
					_content.append("<div class=\"content\">\n");
				...
				case NodeKind_func: {
					...
					if (func->symbol == "outline") {
						_content.append(genOutline());
						break;
					}
					if (func->symbol == "bibliography") {
						_content.append("<div class=\"bibliography\" id=\"");
						_content.append(_escapeId(func->arguments.has("id") ? func->arguments["id"] : ""));
						_content.append("\">\n");
		...
		void _genCode(Node* node) {
			...
			if (codeNode->lang.size() != 0) {
				...
				_content.append(_escape(codeNode->lang));
				_content.append("\">");
				_content.append(_escape(codeNode->content));
		...
		void _genHierarchy(Node* node, String const& openTag, String const& closeTag) {
			HierarchyNode* hierarchyNode = static_cast<HierarchyNode*>(node);
			_content.append(openTag);
			_genArray(hierarchyNode->children);
			_content.append(closeTag);
		}
		...
		void _genFootnote() {
			if (_footnotes.size() != 0) {
				_content.append("<div class=\"footnote-description\">\n");
				for (int i = 0; i < _footnotes.size(); i++) {
					_content.append("<div id=\"Footnote-");
					_content.append(longToString(i + 1));
					_content.append("\"><sup>");
					_content.append(longToString(i + 1));
					_content.append("</sup>");
	...
	struct TemplateSticher {
		String _template;
		String _content;
		Generator& _generator;
		int _index;

		TemplateSticher(Generator& gen, String const& path): _generator{gen} {
			_template = readFile(path);
			_index = 0;
		}

		String stich() {
			if (_template.size() == 0) {
				return _generator.gen();
			}

			_generator.collectLink();

			while (_index < _template.size()) {
				if (_index + 5 < _template.size() &&
					_template[_index] == '<' &&
					_template[_index + 1] == '!' &&
					_template[_index + 2] == '-' &&
					_template[_index + 3] == '-' &&
					...
					_index += 6;
					int start = _index;
					while (_index < _template.size() && _template[_index] != ']') {
						_index++;
					}
					int end = _index;
					if (_index + 3 < _template.size() &&
						_template[_index] == ']' &&
						_template[_index + 1] == '-' &&
						_template[_index + 2] == '-' &&
						_template[_index + 3] == '>') {
					...
					_index += 4;
					String func = _template.slice(start, end - start);
					if (func == "content") {
						_content.append(_generator.gen());
					} else if (func == "outline") {
						_content.append(_generator.genOutline());
					} else if (func == "title") {
						_content.append(_generator.genTitle());
					} else if (func == "metadata") {
						_content.append(_generator.genMetadata());
					}
					continue;
				}
				_content.append(1, _template.items() + _index);
				_index++;
			}
			return _content;
		}
	...
	String gen(Node* node, Option const& option) {
		Generator generator(node, option);
		if (option.template_.size() != 0) {
			TemplateSticher sticher{generator, option.template_};
			return sticher.stich();
		}
