...
double convToFloat(int byteSize, char const* bytes) {
	...
	if (index < byteSize && bytes[index] == '.') {
		int fractionIndex = byteSize - 1;

		while (fractionIndex > index) {
			if (bytes[fractionIndex] == '\'') {
				fractionIndex--;
				continue;
			}

			if (bytes[fractionIndex] >= '0' && bytes[fractionIndex] <= '9') {
				fraction /= 10.0;
				fraction += bytes[fractionIndex] - '0';
				fractionIndex--;
				continue;
			}
		}
